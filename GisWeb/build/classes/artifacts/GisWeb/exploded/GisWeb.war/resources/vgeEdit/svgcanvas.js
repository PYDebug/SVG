...
// 					console.log('s',[start_x, start_y]);
			...
			case "image":
				...
				setHref(newImage, '/GisWeb_GradleEclipse/resource/icon/logo');
...
// 							call("selected", [selected]);
			...
			case "image":
				...
				if($(element).attr("width")==0 && $(element).attr("height")==0){
...
this.setImageURL = function(val,formJson) {
	var elem = selectedElements[0];
	if(formJson){
            (elem).insertAdjacentHTML("beforeBegin","<g id='"+"g_"+elem.id+"' class='entity'>");
            $("#g_"+elem.id).append($("#"+elem.id));
            var gRoot = document.getElementById("g_"+elem.id);
            gRoot.insertAdjacentHTML("beforeEnd","<info>");
            var attrTag = document.getElementById("g_"+elem.id).getElementsByTagName('info')[0];
            for(var key in formJson){
                if(key == "postcode"||key == "phone"||key == "address"){
                	...
                	var elemName = eval("formJson."+key);
                	if(elemName != ""){
                		var xCoor = parseFloat(elem.getAttribute('x'))+10;
                		var yCoor = parseFloat(elem.getAttribute('y'))+25;
                		var attrVal = "<text x="+ xCoor
                			+" y="+ yCoor
                			+" font-size='1' text-anchor='middle'>"
                			+elemName+"</text>";
                		elem.insertAdjacentHTML("afterEnd",attrVal);
                	}
                }
                else{
                    //elem.setAttribute(key, eval("formJson."+key));
                	 var attrVal = "<attr attrname='"+key+"'>"+eval("formJson."+key)+"</attr>";
                     //var attrTag = document.getElementById("g_"+elem.id);
                     attrTag.insertAdjacentHTML("beforeEnd",attrVal);
                }
            }
...
this.editSelectedElements = function(){
        ...
        var ars = new Array('name','postcode','phone','address');
        var lon;
    	var lat;
        //alert(L.id);
        ...
        if($("#attrBox_"+sltEmt).length){
        	...
        	var xCoord = $(L).children()[0].getAttribute('x');
        	var yCoord = $(L).children()[0].getAttribute('y');
        	
        	var lonlat = transCoordinateToLonLat(parseFloat(xCoord),parseFloat(yCoord));
        	lon = lonlat.lon+"";
        	lat = lonlat.lat+"";
	        ...
	        '<table id="attributesBox_'+sltEmt+'" class="attrBoxClass">'+
	            '<tr>'+
	            	'经度:<input id="setLon" class="attr_changer" title="经度" '+
	            	'size="3" data-attr="x" value = "'+lon+'"></input>'+
	            	'纬度:<input id="setLat" class="attr_changer" title="纬度" '+
	            	'size="3" data-attr="y" value = "'+lat+'"></input>'+
	                '<th>属性</th>'+
	                '<th>值</th>'+
	            '</tr>'+
	          '</table>'+
	        ...
	        var tds = function(){
	            ...
	            for (var i = 0; i < ars.length; i++) {
	            	var arEle
	            	if(ars[i] == "name")
	            		arEle = L.getElementsByTagName('text')[0];
	            	else
	            		arEle = L.getElementsByTagName(ars[i])[0];
	                var value = '';
		            	if(arEle) value = arEle.innerHTML;
	            	var tr ='<tr>'+
	                            '<td>'+ars[i]+'</td>'+
	                            '<td><input name="'+ars[i]+'" type="text" value="'+value+'"/></td>'+
	                        '</tr>';
	        ...
	        var attrList = L.getElementsByTagName('attr');
	        
	        for(var i = 0; i< attrList.length ;i++){
	        	var name = attrList[i].getAttribute('attrname');
	        	var value = attrList[i].innerHTML;
        ...
        $("#editButt button:first-child").click(function(){
        	var setLon = $("#setLon").val();
			var setLat = $("#setLat").val();
			
			//alert(setX +" "+ xCoord +" "+ setY +" "+ yCoord);
			if(setLon!="" && setLat!=""){
				if(!(setLon==lon && setLat == lat)){
					var coordinate = transLonLatToCoordinate( parseFloat(setLon),parseFloat(setLat));
					var setX = coordinate.x;
					var setY = coordinate.y;
					
					$("#selected_x:text").val(setX);
					$("#selected_y:text").val(setY);
					svgCanvas.changeSelectedAttribute("x", setX);
					svgCanvas.changeSelectedAttribute("y", setY);
				}
			}else{
				console.log("coordinate null");
			}
        	
			L.getElementsByTagName('info')[0].innerHTML = "";
            for (var i = 0; i < $("#attributesBox_"+sltEmt+" tr input").length; i++) {
                if(i < ars.length){
                	var name = $("#attributesBox_"+sltEmt+" tr input")[i].name;
                	var value = $("#attributesBox_"+sltEmt+" tr input")[i].value;
                	if(name!="name"){
	                	if(value!=""){
	                		var attrVal = "<"+name+">"+value+"</"+name+">";
	                		L.getElementsByTagName('info')[0].insertAdjacentHTML("beforeEnd",attrVal);		
	                	}
                	}else{
                		L.getElementsByTagName('text')[0].innerHTML = value;
                	}
                    //alert(name+" "+value);
                }
                 else{
                    var tgN = $("#attributesBox_"+sltEmt+" td input")[i].value;
                    var tgV = $("#attributesBox_"+sltEmt+" td input")[++i].value;
                    if(tgN != "" || tgV != ""){
                    	var attrVal = "<attr attrname='"+tgN+"'>"+tgV+"</"+name+">";
                    	L.getElementsByTagName('info')[0].insertAdjacentHTML("beforeEnd",attrVal);
                    }
                }
            }
...
/*		width: '100%',
	...
	/*for (var i = 27430; i <= 27448; i++)
		for (var j = 5340; j <= 5352; j++) {
