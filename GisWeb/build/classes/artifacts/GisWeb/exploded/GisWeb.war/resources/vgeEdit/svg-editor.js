...
// 					}
						...
						var ars = new Array('name','postcode','phone','address');
                            var ar ='<div id="attrBox_'+sltEmt+'"><form id="attrForm_'+sltEmt+'">'+
                                    '<table id="attributesBox_'+sltEmt+'" class="attrBoxClass">'+
                                        '<tr>'+
                                        	'经度:<input id="setLon" class="attr_changer" title="经度" size="3" data-attr="x" />'+
                                        	'纬度:<input id="setLat" class="attr_changer" title="纬度" size="3" data-attr="y" />'+
                                            '<th>属性</th>'+
                                            '<th>值</th>'+
                                        '</tr>'+
                                      '</table>'+
			...
			var saveHandler = function(window,svg) {
				...
				var url;
            	var type = localStorage.getItem('flag');
            		...
            		if(xmlHttp.readyState==4 &&xmlHttp.status==200)
            			{
            			 	...
            			 	case "0":
            			 		...
            			 		//alert(resultJSON.mapid);
            			 		localStorage.setItem('layer',resultJSON.mapid);
            			 	...
            			 	case "1":
            			 		result = xmlHttp.responseText;
            			 		var resultJSON = JSON.parse(result);
            			 		//alert(resultJSON.version);
            			 		localStorage.setItem('version',resultJSON.version);
            			 		localStorage.setItem('flag',"2");
            			 		break;
			...
			(function() {
				...
				$('#svgcanvas').bind('mousemove mouseup', function(evt) {
					...
					if(evt.type === 'mouseup') {
						...
						console.log(w_area.scrollLeft+','+w_area.scrollTop+'/'+evt.clientX+','+evt.clientY+'/'+inm_x+','+inm_y);
						//dynamic loading of drag event
...
// 			});
		...
		Editor.loadFromLayer = function(){
			Editor.ready(function(){
				var layerInfo = [];
				var strSVGHead = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">';
				var strSVGTail = '</svg>';
				var content='';
				var loadLayer = function(layer,timeStamp){
					...
					xmlHttp.onreadystatechange = function() {
						...
						if (xmlHttp.readyState==4 && xmlHttp.status==200)
					    {
							...
							tempcontent = tempcontent.replace("\n</g>","").replace("<g>","")
										.replace("<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">","")
										.replace("</svg>","")
				...
				var loadLayerString = function(url){
					var xmlHttp = new XMLHttpRequest();
					var tempcontent;
					//console.log("form outer "+outer);
					xmlHttp.onreadystatechange = function() {
						...
						if (xmlHttp.readyState==4 && xmlHttp.status==200)
					    {
							tempcontent=xmlHttp.responseText;
					    }
					}
				...
				case "0":
					...
					var str = "<svg width=\"2000\" height=\"1000\" xmlns=\"http://www.w3.org/2000/svg\">"
						+"<g><title>"+localStorage.getItem('mapname')+"</title></g></svg>";
					loadSvgString(str);
					break;
				case "1":
					...
					if(content){
						...
						//loadLayer(localStorage.getItem('layer'),localStorage.getItem('timestamp'));
						var tempcontent = loadLayerString(url);
						tempcontent = tempcontent.replace("\n</g>","").replace("<g>","")
						.replace("<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">","")
						.replace("</svg>","")
						.replace("<?xml version=\"1.0\" encoding=\"UTF-8\"?>","")
						...
						//console.log(content);
						content = content + '<g layerid="'+layerId+'"><title>'+layerName+'</title>'+tempcontent+'</g>';
						var str =strSVGHead+content+strSVGTail;
						//console.log(str);
		...
		Editor.saveSuccessCallback = function(){
			alert("succcess");
		}
