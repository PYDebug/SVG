...
public class VTDsvg {
	...
	public String NormalQuery(final String in,ArrayList<String> layerFileLocation,ArrayList<String> newFilePaths) throws ModifyException,
		...
		for(int j=0;j<InToArr.length;j++){
			...
			for(String layerfile:layerFileLocation){
				...
				if(fileFlag){
					...
					String filepath[] = layerfile.split(File.separator);
					String fileout =  filePath;
					fileout = fileout + "/new" + File.separator;
					File cacheFile = new File(new File(fileout) , "test.svg");
					cacheFile.createNewFile();
					fileout = fileout+"test.svg";
