...
(function() {
    ...
    var l = {
            ...
            user: ["Tel","class", "clip-path", "clip-rule", "filter", "height", "id", "mask", "opacity", "requiredFeatures", "style", "systemLanguage", "transform", "width", "x", "xlink:href", "xlink:title", "y"],
...
$.SvgCanvas = function(a, K) {
        ...
        $(a).bind("mousewheel DOMMouseScroll",
            function(h) {
                /*if (h.shiftKey) {*/
                    ...
                    nb = n.getScreenCTM().inverse();
                    //revised
                    var q = oa(event.pageX, event.pageY, nb);
                    q = {
                        x: q.x,
                        y: q.y,
                        width: 0,
                    ...
                    if (h.originalEvent.wheelDelta)
                        if (h.originalEvent.wheelDelta >= 120) q.factor = 2;
                        else {
                            if (h.originalEvent.wheelDelta <= -120) q.factor = 0.5
                        } else if (h.detail)
    ...
    this.setImageURL = function(b,formJson) {
        var c = L[0];
        if(formJson){
            //$(c).wrap("<g id='"+"g_"+c.id+"'></g>");
            //(c).insertAdjacentHTML("beforeBegin", "<g id='"+"g_"+c.id+"'>");
            //(c).insertAdjacentHTML("afterEnd", "</g>");
            (c).insertAdjacentHTML("beforeBegin","<g id='"+"g_"+c.id+"'>");
            $("#g_"+c.id).append($("#"+c.id));
            for(var key in formJson){
                if(key == "postcode"||key == "tel"||key == "address"){
                    var attrVal = "<"+key+">"+eval("formJson."+key)+"</"+key+">";
                    var attrTag = document.getElementById("g_"+c.id);
                    //c.setAttribute(key, eval("formJson."+key));
                    //attrTag.append(attrVal);
    ...
    this.editSelectedElements = function(){
        ...
        //alert(L.toString());
        $("#editBox").show();
        $("#editAttr").append($("#attrBox"));
        $("#editButt button:first-child").click(function(){
            for (var i = 0; i < $("#attributesBox tr input").length-1; i++) {
                if(i < $(L).children().length-1){
                    $(L).children()[i+1].innerHTML = $("#attributesBox tr input")[i].value;
                }
                 else{
                    var tgN = $("#attributesBox td input")[i].value;
                    var tgV = $("#attributesBox td input")[i+1].value;
...
(function() {
    ...
    if (!window.svgEditor) window.svgEditor = function(a) {
        ...
        var l, s = {},
            ...
            k = s.uiStrings = {
                ...
                notification: {
                    ...
                    //finalized: remove new image url
                    enterNewImgURL: "Attribute settings",
        ...
        s.init = function() {
            ...
            function O() {
                /*if($(".tool_button_current").attr("id") == "tool_user"){*/
                    var g = l.getHref(ja);
                    g = g.indexOf("data:") === 0 ? "" : g;
                    //finalized: remove new image url
                    a.prompt(k.notification.enterNewImgURL, g, function(p,q) {
                    p && Bb(p,q)
                })
                /*}*/
            }
            ...
            a.svgIcons(f.imgPath + "svg_edit_icons.svg", {
                //revise: change top_panel icon size
                w: 34,
                h: 34,
                id_match: false,
                ...
                resize: {
                    ...
                    //revise: change layer panel icon size
            ...
            (function() {
                ...
                var g = a("#dialog_box"),
                    ...
                    D = function(J, M, Q, U) {
                        ...
                        p.empty();
                        a("#dialog_container").css({"height":"150px","top":"50%"});
                        a("#dialog_content").css("height","75px");
                        var X = a('<input id="okButton" type="button" value="' + k.common.ok + '">').appendTo(p);
                        J != "alert" && a('<input type="button" value="' + k.common.cancel + '">').appendTo(p).click(function() {
                            g.hide();
                            Q(false)
                        });
                        if (J == "prompt") {
                            //infomation of Attributes
                            var ars = new Array('postcode','tel','address');
                            var ar ='<div id="attrBox"><form id="attrForm">'+ 
                                    '<table id="attributesBox">'+
                                        '<tr>'+
                                            '<th>Attribute</th>'+
                                            '<th>Value</th>'+
                                        '</tr>'+
                                      '</table>'+
                                      '</form>'+
                                      '<div><input type="button" id="addAr" value="Add an attribute" style="margin-bottom:5px"></div><hr /></div>';
                            var tds = function(){
                                var tdstring ='';
                            ...
                            a("#dialog_container").css({"height":"400px","top":"20%"});
                            a("#dialog_content.prompt").css("height","40px");
                            a(ar).prependTo(p);
                            a("#attributesBox").append(tds);
                            var newArNo = 0;
                            a("#addAr").click(function(){                                
                                var att = '<tr id="newAr'+newArNo+'">'+
                                            '<td><input type="text" /></td>'+
                                            '<td><input type="text" /></td>'+
                                            '<td><div onclick="javascript:delAr('+newArNo+')"><img src="images/cancel.png"/></div></td>'+
                                           '</tr>';
                                a("#attributesBox").append(att);
                                newArNo++;
                            });
                        ...
                        X.click(function() {         
                            ...
                            if(J == "prompt"){
                                ...
                                var formJson = getFormJson($('#attrForm'));
                                //alert(formJson.Tel);
                            }
                            var formJson = J == "prompt" ? getFormJson($('#attrForm')) : true;
                            Q && Q(ea,formJson)
                        }).focus();
                        J == "prompt" && ga.focus()
                    };
            ...
            var la = {},
                ...
                Bb = s.setImageURL = function(g,formJson) {
                    //finalized: remove new image url
                if ($(".tool_button_current").attr("id")!="tool_select") {
                    if($("#tools_image_show").attr("data-curopt")=="#tool_image" && $(".tool_button_current").attr("id") !== "tool_user"){
                        g = "images/132placeholder.svg";   
                    }
                    else if($("#tools_image_show").attr("data-curopt")=="#tool_hotel" && $(".tool_button_current").attr("id") !== "tool_user"){
                        g = "images/134house.svg";
                    }
                    else if($(".tool_button_current").attr("id") == "tool_user"){
                        g || (g = kb);
                    }
                };
                    l.setImageURL(g,formJson);
                    a("#image_url").val(g);
                    if (g.indexOf("data:") === 0) {
                        a("#image_url").hide();
                ...
                Na = function() {
                    ...
                    if (g && !M || Ca) {
                        ...
                        sb.enableContextMenuItems("#edit,#delete,#cut,#copy,#move_front,#move_up,#move_down,#move_back")
            ...
            l.bind("saved", function(g, p) {
                ...
                //finalized: support chrome "save map"
                ...
                if (p) {
                    ...
                    a(p).parentsUntil("#svgcontent > g").andSelf().each(function() {
                        if (this.id) {
                            ...
                            var af = p.childNodes[i].tagName;
                            var av = p.childNodes[i].innerHTML;
                            info = info + af+":"+av+"<br/>";
                            };
                            J += " > " + this.id;
                            D += this !== p ? ' > <a href="#">' + this.id + "</a>" : " > " + this.id+"<hr/><b>Attributes</b><br/>"+info
                        }
                    });
                    ua = J
                } else ua = null;
            ...
            var Cb = function() {
                ...
                //finalized: add hotel button
                bh = function(){
                    La("#tool_hotel") && l.setMode("image")
                },
                bu = function(){
                    La("#tool_user") && l.setMode("image")
                },
                db = function() {
                    if (La("#tool_zoom")) {
                        l.setMode("zoom");
                ...
                Oe = function() {
                    if (ja != null || Ca) l.editSelectedElements()
                },
                Nb = function() {
    ...
    K.readLang = function(v) {
        ...
        if (v.tools) {
            ...
            l("title", {
                ...
                //finalized: add hotel button
                tool_hotel: e.mode_hotel,
                tool_user: e.mode_user,
